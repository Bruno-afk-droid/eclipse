...
public class elbow extends Part {
	...
	public elbow(int x, int y,int[] SW,int[] SH,int[] SD, ID id,int px,int py,int pz,GameObject owner,Handler handler) {
		...
		//int PX=Game.sign(px);
		int[] X = {0,16*Game.sign(px),16*Game.sign(px),0};
		int[] Y=  {-4,-4,4,4};
		

		
		int PX=4*Game.sign(px);
	...
	public void tick() {
		...
		//collision_Branch(owner,x-owner.x);
		...
		for(int i=0;i<handler.object.size();i++) {
			GameObject tempObject = handler.object.get(i);	
				...
				if(tempObject.getId() == ID.Ground){
					...
					if(this==TH) {
						Point Dpt = (Game.rotatePoint(new Point(TH.px,TH.py),new Point(0,0),TH.owner.direction));
						double D=Game.Deg(Game.getAngle(new Point(0,0),new Point(Dpt.x,Dpt.y)));
							
							double M = Game.Deg(D-Game.getAngle(new Point(0,0), new Point(Dpt.x+TH.hsp,Dpt.y+TH.vsp)));
							
								DP = 		Game.rotatePoint(new Point(TH.px,TH.py),new Point(0,0),TH.owner.direction+M);
								DPc = 		Game.rotatePoint(new Point(TH.px,TH.py),new Point(0,0),TH.owner.direction);
								if((DPc.x!=DP.x)||(DPc.y!=DP.y)) {
					...
					if(!collision(x,y,tempObject)) {	
						...
						//System.out.println("hsp="+ TH.hsp);
						//System.out.println("vsp="+ TH.vsp);
