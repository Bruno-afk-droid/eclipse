...
public class upper_arm extends Part {
	...
	public upper_arm(int x, int y,int[] SW,int[] SH,int[] SD,int[] layers, ID id,int px,int py,int pz,GameObject owner,Handler handler) {
		...
		//int[] Y=  {-4,-4,4,4};
		/*
		int[] X = {-4,-3, 0, 3,4,3,0,-3};
		int[] Y=  { 0,-3,-4,-3,0,3,4, 3};
		*/
		int[] X = {0,16*Game.sign(px),16*Game.sign(px),0};
		int[] Y=  {-4,-4,4,4};
	...
	public void tick() {
		...
		owner.direction=((direction/720)*360)+(Game.sign(-px)*45);
	...
	public void render(Graphics g) {
		...
		Point Dpt = (Game.rotatePoint(new Point(Zpt.x,py),new Point(0,0),owner.direction));	
